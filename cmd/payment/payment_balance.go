...
func newPaymentBalanceCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "balance",
		Short: "Query payment balances",
	}
	cmd.AddCommand(
		newPaymentBalanceListCmd(),
		newPaymentBalanceGetCmd(),
	)
	return cmd
}

func newPaymentBalanceListCmd() *cobra.Command {
	var onBehalfOf, pageSize, pageNum string
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List payment balances",
		Long: `List all payment account balances.
...
Flags:
  ...
  --on-behalf-of  Sub-account ID to act on behalf of`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			c := client.New(cfg)
			data, err := c.GetHI(context.Background(), "/v2/payment/balances", map[string]string{
				"page_size":   pageSize,
				"page_number": pageNum,
...
func newPaymentBalanceGetCmd() *cobra.Command {
	var onBehalfOf string
	cmd := &cobra.Command{
		Use:   "get <currency>",
		Short: "Get payment balance for a specific currency",
		Long: `Get the payment balance for a specific currency.

The currency argument is the ISO 4217 currency code, e.g. USD, SGD, EUR.

Examples:
  uqpay payment balance get USD`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			c := client.New(cfg)
			data, err := c.GetHI(context.Background(), "/v2/payment/balances/"+args[0], nil,
				map[string]string{"x-on-behalf-of": onBehalfOf})
			if err != nil {
				cmdutil.WriteError(err, cfg.Output)
				return err
